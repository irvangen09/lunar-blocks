				...
				{ rows.map( ( row, index ) => {
					if ( row.isDivider ) {
						return (
							<tr
								key={ index }
								className="lunar-table__row--divider"
							>
								<td
									colSpan={ columns.length }
									className="lunar-table__divider-cell"
								>
									{ row.dividerLabel ?? '' }
								</td>
							</tr>
			...
			{ rows.map( ( row, index ) => {
				if ( row.isDivider ) {
					return (
						<div
							key={ index }
							className="lunar-table__cards-divider"
						>
							{ row.dividerLabel ?? '' }
						</div>
					);
					...
					<div key={ index } className="lunar-table__card">
						<div className="lunar-table__card-head">
							{ image?.url ? (
								...
								<div
									className="lunar-table__card-image-placeholder"
									aria-hidden="true"
								/>
							) }
						...
						{ detailCols.length > 0 && (
							<div className="lunar-table__card-body">
								{ detailCols.map( ( col ) => (
									<div
										key={ col.key }
										className="lunar-table__card-detail"
										data-key={ col.key }
									>
